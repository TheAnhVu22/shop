                    ...
                    <form method="POST" action="{{ route('product.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6 border">
                                <input autocomplete="off" type="text" class="form-control" name="product_content" value="{{ old('product_content') }}" onkeyup="ChangeToSlug();" id="slug">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label for="product_slug" class="col-md-4 col-form-label text-md-right">Slug sản phẩm:</label>
                            <div class="col-md-6 border">
                                <input autocomplete="off" type="text" class="form-control" name="product_slug" value="{{ old('product_slug') }}" id="convert_slug" >
                            </div>
                        </div>
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <select class="custom-select" name="category_id">
                                  @foreach ($category as $key => $dulieu)
                                      <option value="{{$dulieu->id}}">{{$dulieu->category_name}}</option>
                                  @endforeach
                            ...
                            <div class="col-md-6">
                                <select class="custom-select" name="brand_id">
                                  @foreach ($brand as $key => $dulieu)
                                      <option value="{{$dulieu->id}}">{{$dulieu->brand_name}}</option>
                                  @endforeach
                                </select>
                            </div>
                        </div>
